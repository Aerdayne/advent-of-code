...
int is_matching(std::vector<std::string>::const_iterator row) {
  ...
  short first = stoi(buffer[0].substr(0, delimiter_position));
  short second = stoi(buffer[0].substr(delimiter_position + 1));
  char target = buffer[1][0];
  std::string sequence = buffer[2];

  if (sequence[first - 1] != target ^ sequence[second - 1] != target) {
    return 1;
  } else {
    return 0;
  }
